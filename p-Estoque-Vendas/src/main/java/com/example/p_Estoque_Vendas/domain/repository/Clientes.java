...
//      Adiciona um cliente à base de dados. Usa o método persist do EntityManager para
//      adicionar a entidade ao contexto de persistência.
//
//  *atualizar(Cliente cliente)*:
//      Atualiza um cliente na base de dados. Utiliza o método merge do EntityManager para
//      sincronizar as alterações da entidade com o banco de dados.
//
//  *deletar(Cliente cliente)*:
//      Remove um cliente da base de dados. Primeiro verifica se a instância está gerenciada
//      pelo EntityManager. Se não estiver, realiza uma mesclagem (merge) antes de remover.
//
//  *deletar(Integer id)*:
//      Remove um cliente com base no ID fornecido. Chama o método deletar(Cliente cliente)
//      após obter a instância do cliente por meio do EntityManager.
//
//  *buscarPorNome(String nome)*:
//      Realiza uma consulta JPQL para buscar clientes cujos nomes contenham a sequência
//      fornecida. Usa um parâmetro nomeado (:nome) na consulta.
//
//  *obterTodos()*:
//      Obtém todos os clientes da base de dados. Utiliza uma consulta JPQL simples.
//
//
//
//  ### Além disso, observações sobre o código:
//
//  *   Anotações como @Transactional indicam que os métodos devem ser executados em uma
//      transação, e a gestão da transação pode ser providenciada pelo contêiner (por
//      exemplo, um servidor de aplicativos).
//
//  *   Comentários indicam que o EntityManager deveria ser injetado usando a anotação
//      @Autowired, mas essa parte está atualmente comentada. Se estiver utilizando Spring,
//      geralmente você usaria injeção de dependência para obter o EntityManager.
//
//
//  Esse tipo de código é comum em aplicações Java que usam frameworks de persistência
//  para interagir com bancos de dados relacionais. O Hibernate é frequentemente usado em
//  conjunto com o JPA para simplificar as operações de persistência em aplicações Java.
}
